...
#include "FastDeskew.h"
    ...
    {
        return atan2(x,y)*180/3.1415;//求相对原点的角度
    }
    //角度均值尺化操作
    ...
    cv::Mat  correctPlateImage(cv::Mat skewPlate,float angle,float maxAngle)
    {
        ...
        extend_padding = static_cast<int>(skewPlate.rows*tan(cv::abs(angle)/180* 3.14) );//angle具体指代什么意义？
        ...
        if(angle>0) {
            ...
            cv::Mat M  = cv::getPerspectiveTransform(pts1,pts2);
            cv::warpPerspective(skewPlate,dst,M,size,cv::INTER_CUBIC);//opencv透视变换
        }
        return  dst;
    }
            ...
            { for( int i = 0; i < skewImage.cols; i+=blockSize )
                {
                    ...
                    int angle_cell = angle(x2,y2);//获取角度
                    angle_list[(angle_cell + 180)%180]+=1.0;//角度统计0-180
                }
            }
        }
        std::vector<float> filtered = avgfilter(angle_list,5);
        int maxPos = std::max_element(filtered.begin(),filtered.end()) - filtered.begin() + FILTER_WINDOWS_SIZE/2;//统计最多的角度数
        if(maxPos>ANGLE_MAX)
